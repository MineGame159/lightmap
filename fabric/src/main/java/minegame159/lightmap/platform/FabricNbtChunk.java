...
public class FabricNbtChunk implements LightChunk {
    ...
    public FabricNbtChunk(World world, LightChunkPos pos, Codec<ReadableContainer<RegistryEntry<Biome>>> biomeCodec, NbtCompound nbt) {
        this.world = world;
        this.pos = pos;
    ...
    public LightBlock getBlock(int x, int y, int z) {
        return ((IBlockState) getBlockRaw(x, y, z)).lightmap$getBlock();
    }

    private BlockState getBlockRaw(int x, int y, int z) {
        ...
        for (int x = 0; x < 16; x++) {
            for (int z = 0; z < 16; z++) {
                for (int y = maxY; y >= minY; y--) {
                    if (predicate.test(getBlockRaw(x, y, z))) {
